...
export class UpisiSeComponent implements OnInit {
  ...
  prijavljeniKorisnik: JwtUser | null = null;
  ...
  form: FormGroup = this.fb.group({
    ...
    telefon: [
      '',
      [Validators.required, Validators.pattern(/^\d{3}\/\d+$/)]
    ],
    ...
    mestoPrebivalista: [
      '',
      [Validators.required, this.NazivValidator()]
    ],
  ...
  ngOnInit(): void {
    ...
    if (this.token) {
      this.prijavljeniKorisnik =
        this.loginService.decodeToken(this.token);

      if (this.prijavljeniKorisnik?.id != null) {
  ...
  addStudent(): void {
    ...
    if (this.form.invalid || !this.korisnikIzBaze) {
      return;
    }
    ...
    const drzavaObj =
      this.drzave.find(d => d.naziv === f.drzava) || null;

    const mestoZaAdresu =
      this.mesta.find(m => m.naziv === f.mesto) || null;

    const mestoPreb =
      this.mesta.find(
        m => m.naziv === f.mestoPrebivalista
      ) || null;
    ...
    const payload: UpisStudentaRequest = {
      ...
      drzavaNaziv:
        drzavaObj?.id ? null : (f.drzava ?? null),

      mestoId: mestoZaAdresu?.id ?? null,

      mestoNaziv:
        mestoZaAdresu?.id ? null : (f.mesto ?? null),
      ...
      mestoPrebivalistaNaziv:
        mestoPreb?.id
          ? null
          : (f.mestoPrebivalista ?? null)
    };

    if (
      (payload.drzavaId == null ||
        isNaN(Number(payload.drzavaId))) &&
      !payload.drzavaNaziv
    ) {
      this.errorMessage =
        'Morate uneti ili izabrati državu.';
    ...
    if (
      (payload.mestoPrebivalistaId == null ||
        isNaN(Number(payload.mestoPrebivalistaId))) &&
      !payload.mestoPrebivalistaNaziv
    ) {
      this.errorMessage =
        'Morate uneti ili izabrati mesto prebivališta.';
    ...
    this.studentService.upisiStudneta(payload).subscribe({
      ...
      error: (err) => {
        ...
        if (err.status === 409) {
          this.errorMessage =
            err.error?.message ||
            'Već postoji student sa tim JMBG.';
        }

        else if (err.status === 404) {
          this.errorMessage =
            err.error?.message ||
            'Nedostaju referentni podaci.';
        }
          ...
          this.errorMessage =
            'Greška prilikom upisa studenta.';
        }
  ...
  loadMesta(): void {
    ...
    this.mestoService.getAll().subscribe(data => {
      ...
      this.filteredMesta =
        this.form.get('mesto')!.valueChanges.pipe(
          startWith(''),
          map(value =>
            this._filterMesta(value || '')
          )
        );

      this.filteredMestaPreb =
        this.form
          .get('mestoPrebivalista')!
          .valueChanges.pipe(
            startWith(''),
            map(value =>
              this._filterMesta(value || '')
  ...
  loadDrzave(): void {
    ...
    this.drzavaService.getAll().subscribe(data => {
      ...
      this.filteredDrzave =
        this.form.get('drzava')!.valueChanges.pipe(
          startWith(''),
          map(value =>
            this._filterDrzave(value || '')
          )
        );
    });
  }
  ...
  loadAdrese(): void {
    ...
    this.adresaService.getAll().subscribe(adrese => {
      ...
      this.filteredUlice =
        this.form.get('ulica')!.valueChanges.pipe(
          startWith(''),
          map(value =>
            this._filterUlice(value || '')
          )
        );

      this.filteredBrojevi =
        this.form.get('broj')!.valueChanges.pipe(
          startWith(''),
          map(value =>
            this._filterBrojevi(value || '')
          )
        );
    });
  }
  ...
  private _filterMesta(value: string): Mesto[] {
    ...
    return this.mesta.filter(option =>
      option.naziv
        .toLowerCase()
  ...
  private _filterDrzave(value: string): Drzava[] {
    ...
    return this.drzave.filter(option =>
      option.naziv
        .toLowerCase()
        .includes(filterValue)
    );
  ...
  private _filterUlice(
    ...
    return this.ulice.filter(ulica =>
      ulica
        .toLowerCase()
        .includes(filterValue)
    );
  ...
  private _filterBrojevi(
    ...
    return this.brojevi.filter(broj =>
      broj
        .toLowerCase()
        .includes(filterValue)
    );
